...
pub struct LeastSquaresResult<A: Scalar> {
    /// singular values
...
pub trait LeastSquaresSvd<S, A>
where
    S: Data<Elem = A>,
    A: Scalar + Lapack,
{
    fn least_squares(&self, rhs: &ArrayBase<S, Ix1>) -> Result<LeastSquaresResult<A>>;
}

pub trait LeastSquaresSvdInto<S, A>
where
    S: Data<Elem = A>,
    A: Scalar + Lapack,
{
    fn least_squares_into(self, rhs: ArrayBase<S, Ix1>) -> Result<LeastSquaresResult<A>>;
}

pub trait LeastSquaresSvdInPlace<S, A>
where
    S: Data<Elem = A>,
    A: Scalar + Lapack,
{
    ...
    ) -> Result<LeastSquaresResult<A>>;
}

impl<A, S> LeastSquaresSvd<S, A> for ArrayBase<S, Ix2>
where
...
impl<A, S> LeastSquaresSvdInto<S, A> for ArrayBase<S, Ix2>
where
    A: Scalar + Lapack + LeastSquaresSvdDivideConquer_,
    S: DataMut<Elem = A>,
{
    fn least_squares_into(mut self, mut rhs: ArrayBase<S, Ix1>) -> Result<LeastSquaresResult<A>> {
        self.least_squares_in_place(&mut rhs)
    }
}

impl<A, S> LeastSquaresSvdInPlace<S, A> for ArrayBase<S, Ix2>
where
    A: Scalar + Lapack + LeastSquaresSvdDivideConquer_,
    S: DataMut<Elem = A>,
{
